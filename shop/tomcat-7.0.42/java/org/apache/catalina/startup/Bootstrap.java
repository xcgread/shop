 ...
 * @version $Id: Bootstrap.java 1428018 2013-01-02 20:41:24Z markt $ `` * 启动类
 */
...
//      ClassLoaderFactory  中的一个静态内部类, 有2个属性, location, type, 某个位置的某种类型的文件??
	...
	public void init() throws Exception {
		...
		Method method = startupInstance.getClass().getMethod(methodName, paramTypes);// 从Catalina
																						// 类获取setParentClassLoader方法对象,
	...
	public static void main(String args[]) {
		...
		if (daemon == null) {
			...
			try {
				bootstrap.init();// 初始化catalina对象,类加载器等
			} catch (Throwable t) {
				handleThrowable(t);
				t.printStackTrace();
				return;
